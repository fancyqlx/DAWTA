...
def travelFiles_byN(anti):
    ...
    for i in xrange(0,5):
        ...
        if anti:
            filename = "leave_results_N-K="+str(N+60-K-i*10)+"_bits="+str(bits)
        for fn in files:
...
if __name__ == "__main__":
    ...
    leave_x, leave_y, leave_z = travelFiles_byN(1)
    leave_y.reverse()
    leave_z.reverse()
    drawPic(x,y,leave_x,leave_y,"n","Communication Cost (bits)","leave-n-bits")
    drawPic(x,z,leave_x,leave_z,"n","Computation Time (ms)","leave-n-time")
