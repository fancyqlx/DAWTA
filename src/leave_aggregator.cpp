...
int stage4(std::shared_ptr<socketx::Connection> conn, socketx::EventLoop *loop_){
    ...
    if(it+1 == connectionList.end() && connectionList.size()==N-K){
        ...
        ofstream fout("./data/leave_aggregator_logs", std::ofstream::out | std::ofstream::app);
        fout.close();

        fout.open("./data/leave_results_N-K="+std::to_string(N-K)+"_bits="+std::to_string(bits), std::ofstream::out | std::ofstream::app);
...
class Aggregator{
    public:
        ...
        void handleReadEvents(std::shared_ptr<socketx::Connection> conn){
            if(stage==1){
                ofstream fout("./data/leave_aggregator_logs",ofstream::out | ofstream::app);
...
int main(int argc, char **argv){
    ...
    ofstream fout("./data/leave_aggregator_logs");
